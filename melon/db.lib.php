...
function getItem($table, $where="", $order="",$fields="*", $db=null, $viewQuery=false)
{
	...
	if(trim($order)==""){if($melon['column']['index']){$order = $melon['column']['index']." DESC";$query.=' ORDER BY '.$order;}}
...
function getItemJoin($table,$joins,$fields, $where="", $order="", $db=null, $viewQuery=false)
{
	...
	$query .= getWhere($where);
	if(trim($order)==""){if($melon['column']['index']){$order = $table.'.'.$melon['column']['index']." DESC";$query.=' ORDER BY '.$order;}}
	else{$query .= " ORDER BY ".$order;}
	$query .= " LIMIT 1";
